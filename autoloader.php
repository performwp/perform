...
spl_autoload_register(
	function ( $filename ) {
		...
		// Exit if not a class og givewp campaigns plugin.
		if ( 'Perform' !== $filename[0] ) {
			return;
		}

		/**
		 * - The first index will always be Foo since it's part of the plugin.
		...
		$total_file_paths = count( $file_path ) - 1;

		for ( $i = 1; $i < $total_file_paths; $i++ ) {
